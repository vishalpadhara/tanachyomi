...
import '../../models/listhaftorahbook_model.dart';
import '../../utils/current_userinfo.dart';
import '../no_internet_screen.dart';
import '../please_wait_loader.dart';

class HaftorahScreen extends StatefulWidget {
  const HaftorahScreen({Key? key}) : super(key: key);

  @override
  State<HaftorahScreen> createState() => _HaftorahScreenState();
}

class _HaftorahScreenState extends State<HaftorahScreen> {
  ...
  callApi() async {
    ...
    if (isInternet == true) {
      try {
        await callApiLibraryBooksListHafTorah();
      } catch (e) {}
      isLoading = false;
      setState(() {});
    }
                      ...
                      Navigator.push(
                        ...
                        SlideRightRoute(
                          widget: SubListOfLibraryScreen(
                            id: index,
                            name: listHaftorah![index].name,
                            count: 0,
                            bookid: listHaftorah![index].id,
                            IsHaftorahScreen: true,
                          ),
                        ),
                      );
                    },
  ...
  Future<void> callApiLibraryBooksListHafTorah() async {
    ...
    await ApiHandler.callApiLibraryBooksListHafTorah(reference);
    if (reference.isError) {
      reference.isError = false;
    } else {
      try {
        listHaftorah = reference.responseParam;
        print('object');
      } catch (e) {
        print('object');
      }
