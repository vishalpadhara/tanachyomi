...
import '../../models/listwritings_model.dart';
import '../../utils/current_userinfo.dart';
import '../no_internet_screen.dart';
import '../please_wait_loader.dart';

class KesuvimScreen extends StatefulWidget {
  const KesuvimScreen({Key? key}) : super(key: key);

  @override
  State<KesuvimScreen> createState() => _KesuvimScreenState();
}

class _KesuvimScreenState extends State<KesuvimScreen> {
  ...
  List<BookModel>? listWritingsModel;
  static const _chars =
  ...
  callApi() async {
    ...
    if (isInternet == true) {
      try {
        await callApiLibraryBooksListWritings();
      } catch (e) {}
      isLoading = false;
      setState(() {});
    }
  ...
  Consumer<ThemeNotifier> layoutMain() {
    return Consumer<ThemeNotifier>(builder: (context, theme, _) {
      ...
      return Scaffold(
        ...
        body: listWritingsModel != null
            ? Padding(
                padding: const EdgeInsets.only(top: 15.0),
                child: ListView.builder(
                  shrinkWrap: true,
                  itemCount: listWritingsModel!.length /*name.length*/,
                        ...
                        Navigator.push(
                          ...
                          SlideRightRoute(
                            widget: SubListOfLibraryScreen(
                              id: index,
                              name: listWritingsModel![index].name,
                              title: listWritingsModel![index].title,
                              count: 0, //listWritingsModel![index].count,
                              bookid: listWritingsModel![index].id,
                              IsKesuvimScreen: true,
                            ),
                          ),
                        );
                      },
  ...
  Future<void> callApiLibraryBooksListWritings() async {
    ...
    await ApiHandler.callApiLibraryBooksListWritings(reference);
    if (reference.isError) {
      reference.isError = false;
    } else {
      try {
        listWritingsModel = reference.responseParam;
      } catch (e) {}
    }
  }
}
